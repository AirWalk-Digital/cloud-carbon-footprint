...
async function add(cwd, {empty: empty, open: open}, config) {
  const packages = (await getPackages.getPackages(cwd)).packages.filter((pkg => !config.ignore.includes(pkg.packageJson.name))), changesetBase = path__default.default.resolve(cwd, ".changeset");
  let newChangeset;
...
async function internalPublish(pkgName, opts, twoFactorState) {
  ...
  if (0 !== code) {
    ...
    return logger.error(stderr.toString()), {
      published: !1
    };
  }
  return {
